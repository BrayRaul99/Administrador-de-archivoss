...
class Usuarios extends Controller
{
    ...
    public function guardar()
    {
        ...
        if (empty($nombre) || empty($apellido) || empty($correo) || empty($telefono) || empty($clave) || empty($rol)) {
            ...
            if ($id_usuario == '') {
                ...
                if (empty($verificarCorreo)) {
                    ...
                    if (empty($verificarTel)) {
                        ...
                        $data = $this->model->registrar($nombre, $apellido, $correo, $telefono, $hash, $rol);
                        if ($data > 0) {
                ...
                if (empty($verificarCorreo)) {
                    ...
                    if (empty($verificarTel)) {
                        $hash = password_hash($clave, PASSWORD_DEFAULT);
                        $data = $this->model->modificar($nombre, $apellido, $correo, $telefono, $rol, $id_usuario);
                        if ($data == 1) {
    ...
    public function cambiarPass()
    {
        ...
        if (empty($actual) || empty($nueva) || empty($confirmar)) {
            ...
            if ($nueva != $confirmar) {
                $res = array('tipo' => 'error', 'mensaje' => 'LAS CONTRASEÑAS NO COINCIDEN');
            } else {
                ...
                if (password_verify($actual, $consulta['clave'])) {
                    ...
                    $res = array('tipo' => 'error', 'mensaje' => 'CONTRASEÑA ACTUAL INCORRECTA');
                }
            }
        }
        echo json_encode($res, JSON_UNESCAPED_UNICODE);
    ...
    public function cambiarProfile()
    {
        ...
        $apellido = $_POST['apellido'];
        $telefono = $_POST['telefono'];
        if (empty($correo) || empty($nombre) || empty($apellido) || empty($telefono)) {
            $res = array('tipo' => 'error', 'mensaje' => 'TODOS LOS CAMPOS SON REQUERIDOS');
        } else {
            $usuario = $this->model->getUsuario($this->id_usuario);
            $data = $this->model->modificar($nombre, $apellido, $correo, $telefono, $usuario['rol'], $this->id_usuario);
            if ($data == 1) {
